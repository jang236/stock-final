...
class PerfectBatchCollector:
    ...
    def load_companies(self):
        ...
        try:
            ...
            print(f"✅ 전체 {len(companies)}개 종목 로드 완료")
            return companies
    ...
    def visit_main_page(self, stock_code):
        """메인 페이지 방문"""
        try:
    ...
    def extract_parameters(self, page_content):
        ...
        # id 추출 (올바른 Base64 형태) - improved_collector.py와 동일한 로직
        id_patterns = [
            r'[\'"]([A-Za-z0-9+/]{8,})[\'"]'
        ]
    ...
    def test_url_simple(self, url, stock_code):
        """단순 URL 테스트 (HTTP 200만 확인, JSON 파싱 제외)"""
        try:
            ajax_headers = {
            ...
            # HTTP 200이고 접속장애 페이지가 아니면 성공
            if response.status_code == 200 and "접속장애" not in response.text:
                return True
            return False
    ...
    def process_single_company(self, company_name, company_info, index, total):
        ...
        try:
            # 1단계: 메인 페이지 방문
            if not self.visit_main_page(stock_code):
            ...
            # 5단계: 단순 URL 테스트 (JSON 파싱 제외)
            if self.test_url_simple(cf1001_url, stock_code):
                result = {
                    "code": stock_code,
    ...
    def run_batch(self, batch_size=100):
        """100개 배치 실행"""
        print(f"🚀 완벽한 cF1001 URL 데이터베이스 구축")
        print(f"📊 배치 크기: {batch_size}개 | JSON 파싱 체크 제거 (URL 수집 전용)")
        print(f"{'='*70}")
        ...
        if remaining > 0:
            ...
            print(f"\n🚀 다음 실행: python perfect_batch_collector.py")
        else:
            print(f"\n🎉 전체 수집 완료!")
...
def main():
    collector = PerfectBatchCollector()
    collector.run_batch(batch_size=100)

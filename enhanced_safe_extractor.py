...
class NaverRealTimeExtractor:
        ...
        try:
            ...
            result = {
                ...
                'real_time_data': {
                    ...
                    'market_cap': self._extract_market_cap_smart(soup, current_price, stock_code),
    ...
    def _extract_market_cap_smart(self, soup: BeautifulSoup, current_price: Optional[float], stock_code: str = None) -> Optional[str]:
        """시가총액 추출 - API 우선, HTML 스크래핑 폴백"""
        try:
            print("🔍 시가총액 스마트 추출 시도...")
            
            # 1차: 네이버 모바일 API에서 직접 추출 (가장 정확)
            api_result = self._extract_market_cap_from_api(stock_code)
            if api_result:
                print(f"✅ 시가총액 API 추출: {api_result}")
                return api_result
            
            # 2차: HTML 스크래핑 폴백 (기존 로직)
            print("  ⚠️ API 실패, HTML 스크래핑 폴백...")
            ...
            if best_candidate:
                print(f"✅ 시가총액 HTML 추출: {best_candidate}")
    ...
    def _extract_market_cap_from_api(self, stock_code: str) -> Optional[str]:
        """네이버 모바일 API에서 시가총액 직접 추출"""
        try:
            if not stock_code:
                return None

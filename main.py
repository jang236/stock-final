...
async def analyze_company_complete(
    ...
    try:
        ...
        # Enhanced 시스템에서 종목코드 자동 추출 (2,879개 기업 지원)
        try:
            temp_financial = get_financial_data(company_name)
            stock_code = temp_financial.get('company_info', {}).get('code', '005930')
            print(f"✅ Enhanced 매핑: {company_name} → {stock_code}")
        except Exception as e:
            print(f"⚠️ Enhanced 실패, 기본값 사용: {e}")
            stock_code = "005930"

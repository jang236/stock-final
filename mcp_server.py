...
내부 구현은 노출 X — 기존 REST 엔드포인트 함수를 로컬 HTTP로 호출.
"""
import requests
...
# ─────────────────────────────────────────────
# 내부 HTTP 호출 (동일 프로세스 FastAPI)
# ─────────────────────────────────────────────
LOCAL_BASE = "http://localhost:8000"
DEFAULT_TIMEOUT = 30


def _call_local(path: str, params: Optional[dict] = None, timeout: int = DEFAULT_TIMEOUT) -> dict:
    """로컬 FastAPI 엔드포인트 호출. 실패 시 error dict 반환."""
    try:
        resp = requests.get(f"{LOCAL_BASE}{path}", params=params or {}, timeout=timeout)
...
def analyze_company(
    ...
    """
    return _call_local("/analyze-company", {"company_name": company_name, "format": format})
...
def get_stock_price(company_name: str) -> dict:
    ...
    """
    return _call_local(f"/stock/{company_name}", timeout=15)
...
def get_financial_data(company_name: str) -> dict:
    ...
    """
    return _call_local(f"/financial/{company_name}", timeout=30)
...
def get_company_news(company_name: str) -> dict:
    ...
    Returns:
        최신 24시간 + 관련 과거 뉴스 목록.
    """
    return _call_local(f"/news/{company_name}", timeout=15)
...
# ─────────────────────────────────────────────
@mcp.tool()
def get_chart_analysis(symbol: str) -> dict:
    ...
    """
    return _call_local(f"/api/chart/{symbol}/analysis", timeout=15)

...
class NaverRealTimeExtractor:
    ...
    def extract_real_time_data(self, stock_code: str) -> Dict[str, Any]:
        ...
        print(f"🔍 {stock_code} 문제 해결된 범용 시스템 데이터 추출 시작...")
        ...
        try:
            ...
            change_amount = self._extract_change_amount_with_sign_fix(soup)
            change_rate = self._extract_change_rate_with_sign_fix(soup)
            ...
            print(f"✅ 문제 해결된 범용 시스템 데이터 추출 완료!")
            return result
    ...
    def _extract_change_amount_with_sign_fix(self, soup: BeautifulSoup) -> Optional[float]:
        """전일대비 추출 - 부호 오류 해결"""
        try:
            print("🔍 전일대비 부호 수정 추출 시도...")
            
            # 1차: no_exday 영역에서 정확한 부호 감지
            ...
            if exday_area:
                ...
                if blind_elements:
                    ...
                    # 기호가 없는 순수 숫자인 경우 - 개선된 부호 판별
                    elif first_text.replace(',', '').isdigit():
                        try:
                            value = float(first_text.replace(',', ''))
                            
                            # 1. CSS 클래스 기반 부호 판별 (우선)
                            sign = self._detect_sign_by_css(exday_area)
                            if sign is not None:
                                result = value * sign
                                sign_text = "상승" if sign > 0 else "하락"
                                print(f"✅ 전일대비 CSS 부호 감지 ({sign_text}): {result:+,}원")
                                return result
                            
                            # 2. 지역적 기호 검색 (no_exday 주변만)
                            sign = self._detect_sign_local(exday_area)
                            if sign is not None:
                                result = value * sign
                                sign_text = "상승" if sign > 0 else "하락"
                                print(f"✅ 전일대비 지역 부호 감지 ({sign_text}): {result:+,}원")
                                return result
                            
                            # 3. 확실하지 않으면 null 반환 (안전장치)
                            print(f"⚠️ 전일대비 부호 불확실 - 추출 포기")
                            return None
                            
                        except:
                            pass

            # 2차: 기호와 숫자가 함께 있는 패턴 검색
            ...
            for pattern in symbol_patterns:
                ...
                for match in matches:
                    ...
                    # 전일대비 맥락 확인
                    context_start = max(0, match.start() - 50)  # 범위 축소
    ...
    def _detect_sign_by_css(self, exday_area) -> Optional[int]:
        """CSS 클래스 기반 부호 감지"""
        try:
            # 상위 요소들에서 색상 관련 CSS 클래스 찾기
            current = exday_area
            for _ in range(3):  # 3단계까지 상위로 올라가며 검색
                if current:
                    class_list = current.get('class', [])
                    class_str = ' '.join(class_list).lower()
                    
                    # 상승 관련 클래스
                    if any(keyword in class_str for keyword in ['red', 'up', 'plus', 'rise']):
                        return 1
                    # 하락 관련 클래스  
                    elif any(keyword in class_str for keyword in ['blue', 'down', 'minus', 'fall']):
                        return -1
    ...
    def _detect_sign_local(self, exday_area) -> Optional[int]:
        """지역적 부호 감지 (no_exday 주변에서만)"""
        try:
            # exday_area 주변 텍스트만 추출 (200자 이내)
            area_text = exday_area.get_text()
            
            # 다음/이전 형제 요소들도 포함
            siblings = []
            if exday_area.next_sibling:
                siblings.append(str(exday_area.next_sibling))
            if exday_area.previous_sibling:
                siblings.append(str(exday_area.previous_sibling))
            
            local_text = area_text + ' '.join(siblings)
            
            # 지역 텍스트에서 부호 검색
            if any(indicator in local_text for indicator in ['▲', '↑']):
                return 1
            elif any(indicator in local_text for indicator in ['▼', '↓']):
                return -1
    ...
    def _extract_change_rate_with_sign_fix(self, soup: BeautifulSoup) -> Optional[float]:
        """등락률 추출 - 부호 오류 해결"""
        try:
            print("🔍 등락률 부호 수정 추출 시도...")
                    ...
                    if next_sibling and '%' in str(next_sibling):
                        try:
                            ...
                            if rate_text.startswith(('+', '-')):
                                ...
                                print(f"✅ 등락률 기호 포함: {rate:+.2f}%")
                                return rate
                            
                            # 기호가 없는 경우 - CSS나 지역 부호로 판별
                            rate_value = float(rate_text)
                            sign = self._detect_sign_by_css(exday_area) or self._detect_sign_local(exday_area)
                            
                            if sign is not None:
                                result = rate_value * sign
                                print(f"✅ 등락률 부호 감지: {result:+.2f}%")
                                return result
    ...
    def _validate_market_cap_reasonableness(self, market_cap: str, current_price: Optional[float]) -> bool:
        ...
        try:
            ...
            # 현재가 기반 대략적 시가총액 범위 추정 (매우 대략적)
            # 일반적으로 발행주식수는 천만주~수억주 범위
...
def test_fixed_extraction_stock(stock_code: str):
    """문제 해결된 범용 실시간 데이터 추출 테스트"""
    print("🚀 문제 해결된 범용 네이버 실시간 데이터 추출기")
    print("=" * 80)
    ...
    if change_amount is not None and change_rate is not None:
        if change_amount > 0:
            print(f"  ✅ 부호 판별: 상승세 정확 감지")
        else:
            print(f"  ✅ 부호 판별: 하락세 정확 감지")
    
    if real_time['market_cap']:
        if "조" in real_time['market_cap'] or "억" in real_time['market_cap']:
            print(f"  ✅ 시가총액: 스마트 추출 성공")
        else:
            print(f"  ⚠️ 시가총액: 추가 검증 필요")

    # 결과 저장
    import json
    filename = f'fixed_extraction_stock_{stock_code}_result.json'
    ...
    if success_rate >= 95 and calculated_count <= 1:
        print(f"🎉 문제 해결 완료! 완전한 범용성 달성!")
        return True
    elif success_rate >= 87.5:  # 8개 중 7개 이상
        print(f"✅ 크게 개선됨! 안정적인 추출 성능!")
        return True
    else:
        print(f"⚠️ 추가 개선 필요.")
        return False
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        test_fixed_extraction_stock(stock_code)
    else:
        print("📋 사용법:")
        print("python fixed_extractor.py [종목코드]")
        print("\n🔧 문제 해결된 핵심 8개 항목:")
        print("  ✅ 현재가, 전일대비, 등락률 (부호 오류 해결)")
        print("  ✅ 거래량, 거래대금")
        print("  ✅ 시가총액 (소형주 오류 해결)")
        print("  ✅ 외국인지분율, 동종업종")
        print("\n🎯 해결된 문제들:")
        print("  🔧 부호 판별: CSS 클래스 + 지역 검색 + 안전장치")
        print("  🔧 시가총액: 규모별 스마트 처리 + 합리성 검증")
        print("\n추천 테스트 종목:")
        print("  046890 (서울반도체) - 소형주 부호 오류 해결 확인")
        print("  213420 (달바글로벌) - 소형주 시가총액 오류 해결 확인")
        print("  035720 (카카오) - 중형주 성능 유지 확인")
        print("  000660 (SK하이닉스) - 대형주 성능 유지 확인")
        print("\n예시:")
        print("  python fixed_extractor.py 046890")

...
class ImprovedCF1001BatchCollector:
    ...
    def test_cf1001_url(self, url, stock_code):
        """cF1001 URL 유효성 테스트 - 관대한 검증 (URL 수집 전용)"""
        try:
            # AJAX 헤더 설정
            ajax_headers = {
    ...
    def collect_batch(self, batch_size=30):
        """배치 수집 - 성공했던 로직에 배치 기능 추가 (속도 최적화 적용)"""
        print(f"🚀 속도 최적화된 cF1001 수집기 (3.6배 빨라짐!)")
        print(f"📊 배치 크기: {batch_size}개")
        print(f"⚡ 대기시간: 2-3.5초 랜덤 (기존 10초)")
        print(f"{'='*70}")
        ...
        estimated_seconds = remaining_companies * 2.75  # 평균 2.75초
        estimated_hours = estimated_seconds / 3600
        print(f"   남은 종목: {remaining_companies:,}개")
        print(f"   예상 소요시간: {estimated_hours:.1f}시간 (기존 대비 3.6배 빨라짐)")
        print(f"{'='*70}")
        ...
        for i, (company_name, company_info) in enumerate(current_batch):
            ...
            # 마지막이 아니면 랜덤 대기 (속도 최적화)
            if i < len(current_batch) - 1:
                sleep_time = random.uniform(2.0, 3.5)
                print(f"   ⏰ 다음 종목까지 {sleep_time:.1f}초 대기... (로봇 탐지 회피)")
                time.sleep(sleep_time)
        ...
        # 속도 개선 효과 표시
        batch_time = len(current_batch) * 2.75  # 평균 대기시간
        old_time = len(current_batch) * 10  # 기존 대기시간
        time_saved = old_time - batch_time
        print(f"⚡ 시간 절약: {time_saved/60:.1f}분 (기존 대비 {old_time/batch_time:.1f}배 빨라짐)")
        ...
        if remaining > 0:
            ...
            print(f"⚡ 전체 완료까지: 약 {estimated_total_hours:.1f}시간 (기존 방식 대비 3.6배 빨라짐)")
            print(f"🚀 다음 실행: python fixed_batch_collector.py")
        else:
            print(f"🎉 전체 수집 완료!")
